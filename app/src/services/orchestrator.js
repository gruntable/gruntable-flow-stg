 ...
 * @param {string} workflow_id - The project/workflow ID, used to scope table names per workflow
 * @returns {Promise<Object>} Run configuration with run_guid and nodes array.
 *   Each node includes trigger_type ("form"|"auto"), and either:
 *   - form_url (string) for form-based nodes (ai_extraction)
 *   - webhook_url + webhook_body (object) for auto-triggered nodes (ai_transformation, export_excel)
 */
export const startRun = async (trigger_mode, nodes, workflow_id) => {
  const response = await fetch(ORCHESTRATOR_ENDPOINTS.START, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ trigger_mode, nodes, workflow_id }),
  });
