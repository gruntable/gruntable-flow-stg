...
export default function InputPanel({
  ...
  pauseReviewTableName, pauseReviewTableLoading, pauseReviewTableError, pauseReviewTableData,
  showPauseReview,
  editMode,
  isStarting,
}) {
    ...
    <div style={{ ...S.panel, width, minWidth: width }}>
      {!isViewOnly && (
        <div style={{ display: "flex", borderBottom: `1px solid ${C.border}` }}>
          ...
          <button onClick={() => setMidTab("run")} style={{
            ...S.tab(midTab === "run"),
            background: showPauseReview ? "#f0fdf4" : orchestratorStatus === 'running' ? "#eff6ff" : "transparent",
          }}>▶ Run</button>
        </div>
      )}
      {isViewOnly && (
      ...
      {orchestratorStatus !== 'idle' && (
        <div style={{
          ...
          {pauseReviewTableName && <span>table: <b>{pauseReviewTableLoading ? 'loading...' : pauseReviewTableError ? 'ERROR' : pauseReviewTableData ? 'loaded' : 'pending'}</b></span>}
          {pauseReviewTableName && <span title={pauseReviewTableName}>{getCleanTableName(pauseReviewTableName).length > 20 ? getCleanTableName(pauseReviewTableName).slice(0, 20) + '...' : getCleanTableName(pauseReviewTableName)}</span>}
        </div>
      )}
    </div>
  );
