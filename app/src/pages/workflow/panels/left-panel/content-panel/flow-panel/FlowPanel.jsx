...
import { C, S, BICON } from "../../../../../../styles.jsx";
...
export default function FlowPanel({
  ...
  const isRunning = orchestratorStatus === 'running' || orchestratorStatus === 'node_done' || isPolling;
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 12, paddingBottom: 0 }}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              {(orchestratorStatus === 'running' || orchestratorStatus === 'node_done') ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <button
                    onClick={undo}
                    disabled={!canUndo || !editMode || isLoading}
                    title="Undo (Ctrl+Z)"
                    style={{
                      ...S.btnGhost,
                      padding: "4px 8px",
                      ...
                      fontSize: 11,
                      opacity: (canUndo && editMode) || isLoading ? 1 : 0.5,
                  ...
                  <button
                    onClick={redo}
                    disabled={!canRedo || !editMode || isLoading}
                    title="Redo (Ctrl+Y)"
                    style={{
                      ...S.btnGhost,
                      padding: "4px 8px",
                      ...
                      fontSize: 11,
                      opacity: (canRedo && editMode) || isLoading ? 1 : 0.5,
            ...
            <button
              ...
              style={{ ...S.btnP, width: "100%", opacity: isRunning || isStarting || isLoading ? 0.5 : 1, fontSize: 12 }}
              title={!allNodesReady && firstInvalidNode ? invalidErrors.join(", ") : ""}
            >
              {isLoading ? '⏳ Loading...' : !allNodesReady ? '⚠️ Check Required Fields' : isRunning ? '⏳ Running...' : isStarting ? '⏳ Starting...' : '▶ Run All Steps'}
            </button>
          ...
          {isLoading && nodes.length === 0 ? (
            <div>
              {[0, 1, 2].map(i => (
                <div key={i}>
                  <div style={{ ...shimmerStyle, height: 52, borderRadius: 6, border: '1px solid #e0e0e0', marginBottom: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px' }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#d1d5db' }} />
                      <div style={{ ...shimmerStyle, width: '50%', height: 12, borderRadius: 4 }} />
                    </div>
                    <div style={{ height: 4 }} />
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px' }}>
                      <div style={{ width: 14 }} />
                      <div style={{ ...shimmerStyle, width: '30%', height: 8, borderRadius: 4 }} />
                    </div>
                  </div>
                  {i < 2 && (
                    <div style={{ position: "relative", textAlign: "center", height: 28, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            return (
              <div key={node.id} data-node-id={node.id}>
                <div
                  ...
                  style={{ ...S.nodeCard(sel === node.id, executing, completed, false), cursor: isRunning || isViewOnly ? 'not-allowed' : canDrag ? 'grab' : 'pointer', opacity: isViewOnly ? 0.7 : isDragged ? 0.3 : (isRunning && !executing && !completed) ? 0.4 : 1 }}
                >
