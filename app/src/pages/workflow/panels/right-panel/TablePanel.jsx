...
// Right-side panel. Table data is lifted to Workflow.
// ─────────────────────────────────────────────
...
function calcInitialWidths(headers, rows) {
  const CHAR_PX = 8;
  const MAX_W = 400;
  const sampleRows = rows.slice(0, 20);
  return headers.map((h, ci) => {
    const headerLen = (h || `Col ${ci + 1}`).length;
    const maxLen = sampleRows.reduce((max, row) => {
      return Math.max(max, String(row[ci] ?? "").length);
    }, headerLen);
    return Math.max(COL_MIN_W, Math.min(MAX_W, maxLen * CHAR_PX + 24));
  });
}
...
const TableView = memo(function TableView({ table, onChange }) {
  ...
  const [colWidths, setColWidths] = useState(() => calcInitialWidths(headers, rows));
  ...
  const [colDragPos, setColDragPos] = useState(null);
  const [colDropIndicator, setColDropIndicator] = useState(null);
  ...
  const moveCol = (fromCi, toCi) => {
    if (fromCi === toCi) return;
    const newHeaders = [...headers];
    const [moved] = newHeaders.splice(fromCi, 1);
    newHeaders.splice(toCi, 0, moved);
    const newRows = rows.map(r => {
      const newRow = [...r];
      const [movedCell] = newRow.splice(fromCi, 1);
      newRow.splice(toCi, 0, movedCell);
      return newRow;
    });
    const newColWidths = [...colWidths];
    const [movedWidth] = newColWidths.splice(fromCi, 1);
    newColWidths.splice(toCi, 0, movedWidth);
    onChange({ ...table, headers: newHeaders, rows: newRows });
  ...
  const startColDrag = (e, ci) => {
    ...
    const startX = e.clientX;
    const startY = e.clientY;
    colDragRef.current = { ci, startX, startY, hasDragged: false };

    const onMove = (ev) => {
      if (!colDragRef.current) return;
      if (!colDragRef.current.hasDragged) {
        if (Math.abs(ev.clientX - colDragRef.current.startX) < 5) return;
        colDragRef.current.hasDragged = true;
        setDraggedCol(colDragRef.current.ci);
      }
      colDragRef.current.currentX = ev.clientX;
      colDragRef.current.currentY = ev.clientY;
      setColDragPos({ x: ev.clientX, y: ev.clientY });

      const el = document.elementFromPoint(ev.clientX, ev.clientY);
      const thEl = el?.closest('th[data-col-index]');
      if (thEl) {
        const targetCi = parseInt(thEl.dataset.colIndex, 10);
        if (targetCi !== colDragRef.current.ci) {
          const rect = thEl.getBoundingClientRect();
          const midX = rect.left + rect.width / 2;
          const onLeft = ev.clientX < midX;
          const draggedCi = colDragRef.current.ci;
          const wouldBeNoChange = (onLeft && targetCi >= draggedCi) || (!onLeft && targetCi <= draggedCi);
          if (wouldBeNoChange) {
            colDragRef.current.dropTarget = null;
            colDragRef.current.dropSide = null;
            setColDropIndicator(null);
          } else {
            colDragRef.current.dropTarget = targetCi;
            colDragRef.current.dropSide = onLeft ? 'before' : 'after';
            setColDropIndicator({
              left: onLeft ? rect.left : rect.right,
              top: rect.top,
              height: rect.height,
            });
          }
    ...
    const onUp = () => {
      ...
      window.removeEventListener("pointerup", onUp);
      if (colDragRef.current?.hasDragged) {
        const el = document.elementFromPoint(colDragRef.current.currentX, colDragRef.current.currentY);
        const thEl = el?.closest('th[data-col-index]');
        if (thEl) {
          const targetCi = parseInt(thEl.dataset.colIndex, 10);
          const draggedCi = colDragRef.current.ci;
          const dropSide = colDragRef.current.dropSide;
          const wouldBeNoChange = (dropSide === 'before' && targetCi >= draggedCi) || (dropSide === 'after' && targetCi <= draggedCi);
          if (!wouldBeNoChange) {
            const dropIdx = dropSide === 'after' ? targetCi + 1 : targetCi;
            moveCol(draggedCi, dropIdx);
          }
        }
      }
  ...
  // Ghost element for column drag
  const colGhost = draggedCol != null && colDragPos && createPortal(
    <>
      <div style={{
        position: "fixed",
        left: colDragPos.x + 8, top: colDragPos.y - 14,
        padding: "6px 12px",
        ...
        {headers[draggedCol] || `Col ${draggedCol + 1}`}
      </div>
      {colDropIndicator && (
        <div style={{
          position: "fixed",
          left: colDropIndicator.left - 1,
          top: colDropIndicator.top,
          width: 2,
          height: colDropIndicator.height,
          background: C.black,
          borderRadius: 1,
          pointerEvents: "none",
          zIndex: 99999,
  ...
  return (
    ...
    <div style={{ flex: 1, overflow: "auto", position: "relative" }}>
      <table style={{
        ...
        width: totalW, tableLayout: "fixed",
      }}>
        <colgroup>
          <col style={{ width: 40 }} />
          {headers.map((_, ci) => (
          ...
          <tr style={{ background: "#f0f0f0", userSelect: "none" }}>
            ...
            {headers.map((h, ci) => (
              <th
                ...
                style={{ ...thStyle("#f0f0f0"), position: "relative", padding: 0, overflow: "visible", opacity: draggedCol === ci ? 0.3 : 1 }}
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "stretch", height: ROW_H, overflow: "visible", position: "relative" }}>
              ...
              {ri > 0 && (
                ...
                >
                  ...
                  <td
                    ...
                    onPointerDown={e => startRowResize(e, ri - 1)}
                  >
                    <div
                      style={{
...
const TablePanel = memo(function TablePanel({ tables, activeTable, onChangeActive, onChangeTable, onCloseTable, onCloseAllTables, onOpenTable, onCreateTable, workflowId, loadingTableName, orchestratorStatus, isPolling, pollCount, onDeleteTable, onRenameTable, pauseReviewTableDirty, pauseReviewTableSaving, handleSaveTable }) {
  ...
  const [tabOrder, setTabOrder] = useState(() => {
    if (workflowId) {
      try {
        const saved = JSON.parse(localStorage.getItem(`tabOrder_${workflowId}`));
        if (Array.isArray(saved)) {
          const existing = saved.filter(n => n in tables);
          const added = Object.keys(tables).filter(n => !saved.includes(n));
          if (existing.length > 0) return [...existing, ...added];
        }
      } catch {}
    }
    return Object.keys(tables).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
  });
  const [draggedTab, setDraggedTab] = useState(null);
  const [dragPos, setDragPos] = useState(null); // { x, y } for ghost
  const [dropIndicator, setDropIndicator] = useState(null); // { left, top, height } for line
  const tabDragRef = useRef(null);

  const startTabDrag = useCallback((e, name) => {
    if (e.button !== 0) return;
    ...
    const startX = e.clientX;
    const startY = e.clientY;
    tabDragRef.current = { name, startX, startY, hasDragged: false };

    const onMove = (ev) => {
      if (!tabDragRef.current) return;
      if (!tabDragRef.current.hasDragged) {
        if (Math.abs(ev.clientX - tabDragRef.current.startX) < 5) return;
        tabDragRef.current.hasDragged = true;
        setDraggedTab(tabDragRef.current.name);
      }
      setDragPos({ x: ev.clientX, y: ev.clientY });
      // Find drop target and compute indicator position
      const el = document.elementFromPoint(ev.clientX, ev.clientY);
      const tabEl = el?.closest('[data-tab-name]');
      if (tabEl && tabEl.dataset.tabName !== tabDragRef.current.name) {
        const rect = tabEl.getBoundingClientRect();
        const midX = rect.left + rect.width / 2;
        const onLeft = ev.clientX < midX;
        tabDragRef.current.dropTarget = tabEl.dataset.tabName;
        tabDragRef.current.dropSide = onLeft ? 'before' : 'after';
        setDropIndicator({
          left: onLeft ? rect.left : rect.right,
          top: rect.top,
          height: rect.height,
        ...
        tabDragRef.current.dropTarget = null;
        tabDragRef.current.dropSide = null;
    ...
    const onUp = () => {
      ...
      window.removeEventListener('pointerup', onUp);
      if (tabDragRef.current?.hasDragged) {
        const { dropTarget, dropSide, name: dragged } = tabDragRef.current;
        if (dropTarget && dropTarget !== dragged) {
          setTabOrder(prev => {
            const next = prev.filter(n => n !== dragged);
            let idx = next.indexOf(dropTarget);
            if (dropSide === 'after') idx += 1;
  ...
  // Sync tabOrder when tables are added/removed
  const tableKeys = Object.keys(tables).sort().join('\0');
  useEffect(() => {
    setTabOrder(prev => {
      const existing = prev.filter(n => n in tables);
      const added = Object.keys(tables).filter(n => !prev.includes(n));
      if (added.length === 0 && existing.length === prev.length) return prev;
      // If no existing tabs survived (e.g. tables loaded async after init), restore from localStorage
      if (existing.length === 0 && workflowId) {
        try {
          const saved = JSON.parse(localStorage.getItem(`tabOrder_${workflowId}`));
          if (Array.isArray(saved)) {
            const restored = saved.filter(n => n in tables);
            const extra = Object.keys(tables).filter(n => !saved.includes(n));
  ...
  }, [tableKeys, workflowId]);

  // Persist tab order to localStorage
  useEffect(() => {
    if (workflowId && tabOrder.length > 0) {
      localStorage.setItem(`tabOrder_${workflowId}`, JSON.stringify(tabOrder));
    }
  }, [tabOrder, workflowId]);

  // Refs to stabilize handleTableChange — prevents infinite re-renders of TableView
  const tablesRef = useRef(tables);
  tablesRef.current = tables;
  const onChangeTableRef = useRef(onChangeTable);
  onChangeTableRef.current = onChangeTable;
  ...
  const current = (activeTable && tables[activeTable])
    ? activeTable
    : names.find(n => tables[n]) ?? names[0];
  ...
  const handleTableChange = useCallback((data) => {
    setHistory(h => {
      ...
      return {
        ...h,
        [current]: {
          past: [...prev.past.slice(-49), tablesRef.current[current]],
          future: [],
        }
      };
    });
    onChangeTableRef.current(current, data);
  }, [current]);
  ...
  const handleRenameSubmit = useCallback(() => {
    ...
    const currentClean = getCleanTableName(renameModalOpen);
    if (trimmed === currentClean) { setRenameModalOpen(null); return; }
    const oldName = renameModalOpen;
    const prefixMatch = oldName.match(/^([\w-]+_[a-z0-9]+_)/);
    const newTechnicalName = prefixMatch
      ? prefixMatch[1] + trimmed
      : trimmed;
    if (Object.keys(tables).includes(newTechnicalName)) {
      setRenameError("A table with this name already exists");
      return;
    }
    setTabOrder(prev => prev.map(n => n === oldName ? newTechnicalName : n));
    onRenameTable(oldName, newTechnicalName);
    setRenameModalOpen(null);
  }, [renameValue, renameModalOpen, tables, onRenameTable]);
  ...
  return (
    <>
      ...
      {/* Floating ghost tab & drop indicator during drag */}
      {draggedTab && dragPos && createPortal(
        <>
          {/* Ghost tab following cursor */}
          <div style={{
            position: "fixed",
            left: dragPos.x + 8, top: dragPos.y - 14,
            padding: "6px 12px",
          ...
          {names.map(name => {
            ...
            return (
              <Fragment key={name}>
                <button
                  data-tab-name={name}
                  onPointerDown={(e) => startTabDrag(e, name)}
                  style={{
                    flexShrink: 0,
                    padding: "8px 14px",
        ...
        {onRenameTable && (
          <button
            onClick={(e) => {
              ...
              setRenameValue(getCleanTableName(name));
              setRenameError("");
              setRenameModalOpen(name);
              setMenuOpenForTab(null);
              setMenuPos(null);
            }}
            style={{
    ...
    {renameModalOpen && onRenameTable && createPortal(
      <div
      ...
      >
        <div
          onClick={e => e.stopPropagation()}
          style={{ background: C.white, borderRadius: 10, width: 380, boxShadow: "0 8px 32px rgba(0,0,0,0.15)", padding: "20px 22px", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}
        >
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Rename Table</div>
          <label style={{ fontSize: 11, fontWeight: 600, color: C.mid, marginBottom: 3, display: "block" }}>New name</label>
          <input
            value={renameValue}
            onChange={e => { setRenameValue(e.target.value); setRenameError(""); }}
            onKeyDown={e => {
              if (e.key === "Enter") { e.preventDefault(); handleRenameSubmit(); }
              if (e.key === "Escape") setRenameModalOpen(null);
            }}
            placeholder="Table name"
            style={S.input}
            autoFocus
          />
          {renameError && <div style={{ fontSize: 12, color: C.red, marginTop: 6 }}>{renameError}</div>}
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16 }}>
            <button onClick={() => setRenameModalOpen(null)} style={S.btnS}>Cancel</button>
            <button onClick={handleRenameSubmit} style={S.btnP}>Rename</button>
          </div>
        </div>
      </div>,
      document.body
