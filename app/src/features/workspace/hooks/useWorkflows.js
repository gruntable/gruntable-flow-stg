  ...
  const handleFlowImport = (event, onImported) => {
    ...
    reader.onload = () => {
      try {
        ...
        triggerSave();
        setWorkflows(metadata.workflows);
        setActiveWorkflowId(metadata.activeWorkflowId);
        setShowWorkflowMenu(false);
        setWorkflowEditMode(false);
        setFlowNoticeTone("neutral");
        setFlowNotice(`Flow Import complete (${metadata.workflows.length} workflow${metadata.workflows.length > 1 ? "s" : ""}).`);
