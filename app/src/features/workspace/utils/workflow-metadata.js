...
export const normalizeWorkflow = (rawProject, idx) => {
  ...
  const id = typeof rawProject?.id === "string" && rawProject.id.trim() ? rawProject.id : crypto.randomUUID();
...
export const convertNodesToOrchestratorFormat = (nodes, tableNames, nodePrompts, nodeSourceTables, allTables = []) => {
  const result = nodes
    .map((node, index) => {
      ...
      // Handle export_excel node - uses single source table from nodeSourceTables
      let sourceTableName = null;
      if (node.node_type === 'export_excel') {
        const stored = nodeSourceTables[node.id];
        if (typeof stored === 'string' && stored.trim()) {
          sourceTableName = stored.trim();
        } else if (Array.isArray(stored) && stored[0]) {
          sourceTableName = stored[0].trim();
        }
        console.log(`[WF-META] Export Excel node "${node.label}" - sourceTableName:`, sourceTableName);
      }

      // Handle ai_export node - uses single source table from nodeSourceTables
      if (node.node_type === 'ai_export' || node.behavior === 'ai_export') {
        const stored = nodeSourceTables[node.id];
        if (typeof stored === 'string' && stored.trim()) {
          sourceTableName = stored.trim();
        } else if (Array.isArray(stored) && stored[0]) {
        ...
        console.log(`[WF-META] AI Export node "${node.label}" - sourceTableName:`, sourceTableName);
      }
      ...
      return {
        ...
        fileName: (node.behavior === 'basic_export' || isAiExport || node.behavior === 'export')
        ...
        extraction_mode: node.node_type === 'ai_extraction'
          ...
          : undefined,
        conflict_mode: conflictMode, // overwrite | append
        source_table_name: sourceTableName, // for export_excel and ai_export nodes
      };
    });
